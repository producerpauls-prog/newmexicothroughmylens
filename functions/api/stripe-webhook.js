...
export async function onRequestPost(context) {
  ...
  const imageUrl = photoUrl(photoNumber, env.PUBLIC_SITE_URL || 'https://newmexicothroughmylens.com');
  ...
  const payload = {
    ...
    items: [{
      merchantReference: `${photoNumber}-${size}`,
      sku,
      copies: 1,
...
function getPhotoNumber(session) {
  const fromRef = (session.client_reference_id || '').match(/NM-\d{3}/i)?.[0];
  if (fromRef) return fromRef.toUpperCase();
  for (const field of session.custom_fields || []) {
    const label = field.label?.custom || '';
    if (/photo\s*number/i.test(label)) {
      const value = field.text?.value || field.numeric?.value || field.dropdown?.value || '';
      const match = String(value).match(/NM-?\d{3}|\d{1,3}/i);
      if (match) {
        const n = String(match[0]).replace(/\D/g, '');
        return `NM-${n.padStart(3, '0')}`;
      }
    }
  }
  return null;
...
function parseSizeFromReference(ref = '') {
  const normalized = ref.toUpperCase().replace(/[^0-9X]/g, '');
  for (const size of ['8X10','11X14','16X20','20X30']) if (normalized.includes(size)) return size;
  return null;
}
...
function photoUrl(photoNumber, base) {
  ...
  const file = files[photoNumber];
  return file ? `${base.replace(/\/$/, '')}/images/portfolio/${file}` : null;
}
