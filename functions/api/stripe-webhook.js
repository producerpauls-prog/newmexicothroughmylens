...
const DEFAULT_PRINT_ASSET_BASE_URL = 'https://newmexicothroughmylens-site.pages.dev';
...
export async function onRequestPost(context) {
  ...
  // Use the stable Pages hostname so Prodigi's downloader bypasses any custom-domain
  // WAF/hotlink rules. The storefront continues to use its existing public domain.
...
function photoUrl(photoNumber, base) {
  ...
  const file = files[photoNumber]; return file ? `${base.replace(/\/$/, '')}/images/portfolio/${file}` : null;
}
